import { convertSnowflakeToDate } from "@riskybot/tools";
...
export default class User {
    ...
    get createdAt(): Date {
        return new Date(convertSnowflakeToDate(this.id));
    }
    ...
    avatarURL(config: {extension: UserAvatarFormat, size: number } = {extension: ImageFormat.PNG, size: 512}): string | null {
        ...
        return DISCORD_IMAGE_BASE_URL + CDNRoutes.userAvatar(this.id, this.avatar, config.extension) + `?size=${config.size}`;
    }
    ...
    bannerURL(config: {extension: UserBannerFormat, size: number } = {extension: ImageFormat.PNG, size: 512}): string | null {
        ...
        return  DISCORD_IMAGE_BASE_URL+ CDNRoutes.userBanner(this.id, this.banner, config.extension) + `?size=${config.size}`;
    }
    ...
    get defaultAvatarURL(): string {
        ...
        return DISCORD_IMAGE_BASE_URL+ CDNRoutes.defaultUserAvatar(discriminatorModulo);
    }
